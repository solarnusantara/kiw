...
class AizUploadController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        if ($request->hasFile('aiz_file')) {
            ...
            if (isset($type[$upload->extension])) {
              if (in_array($upload->extension, ['jpg', 'jpeg', 'png', 'svg', 'webp', 'gif'])) {
                $file = $request->file('aiz_file'); 
                $bannerContents = file_get_contents($file->getRealPath());
                $bannerName = pathinfo($upload->file_original_name, PATHINFO_FILENAME);
                $bannerName = preg_replace('/[^A-Za-z0-9-]+/', '-', $bannerName);
                $bannerName = ltrim($bannerName, '-');
                $bannerName .= '.webp';
                ...
                Storage::put('uploads/all/' . $bannerName, $image); 
                

 
                $upload->file_name = 'uploads/all/' . $bannerName; 
                $upload->user_id = Auth::user()->id; // Assuming user_id is 1 for this example
    ...
    public function get_uploaded_files(Request $request)
    {
        ...
        // dd($uploads->paginate(60)->appends(request()->query()));
    ...
    {
        $file = Upload::with('user')->findOrFail($request['id']);
        // dd($file);
