    ...
    public function order_store(Request $request)
    {
        ...
        if (Session::has('pos.cart') && count(Session::get('pos.cart')) > 0) {
            ...
            if ($combined_order->save()) {
				...
				if ($request->payment_type === 'xendit') { 
					$p = new Payment();
					$p->user_id = $user->id;
					$p->order_id = $order->id; // Add order_id to Payment
					$p->external_id = $combined_order->code;
					$p->amount = $grand_total;
					$p->payer_email = $user->email;
					$p->description = 'Order Placed. Order Code ' . $combined_order->code;

					$client = new Client();
					$response = $client->post('https://api.xendit.co/v2/invoices', [
						'auth' => [env('XENDIT_ACCESS_TOKEN'), ''],
						'json' => [
							'external_id' => $combined_order->code,
							'amount' => $combined_order->grand_total,
							'payer_email' => $user->email,
							'description' => 'Order Placed. Order Code ' . $combined_order->code,
							'invoice_duration' => 172800
						],
					]);

					$generateInvoice = json_decode($response->getBody(), true);
					$p->checkout_link = $generateInvoice['invoice_url'];
					$p->save();
