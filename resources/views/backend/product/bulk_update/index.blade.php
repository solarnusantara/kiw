...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            <br>
            <div class="">
                <a href="{{ static_asset('download/product_bulk_demo_edit.xlsx') }}" download><button
                        class="btn btn-info">{{ translate('Download CSV') }}</button></a>
                <a href="{{ static_asset('download/sample_bulk_update_taxes.xlsx') }}" download><button
                        class="btn btn-info">{{ translate('Download CSV for TAX') }}</button></a>
            </div>
            <div class="alert"
                style="color: #004085;background-color: #cce5ff;border-color: #b8daff;margin-bottom:0;margin-top:10px;">
            ...
            <div class="">
                ...
                <a href="{{ route('pdf.download_tax') }}"><button
                        class="btn btn-info">{{ translate('Download Tax') }}</button></a>
            </div>
            <br>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('bulk_product_update') }}" method="POST"
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6"><strong>{{ translate('Upload Product File For Tax') }}</strong></h5>
        </div>
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('bulk_product_update_tax') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="form-group row">
                    <div class="col-sm-9">
