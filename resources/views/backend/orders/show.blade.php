                ...
                <div class="card-header">
                    <div class="flex-grow-1 row align-items-start">
                        ...
                        <div class="col-md-3 mr-auto ml-0">
                            ...
                            <div class="mb-3">
                                <label class="mb-0">{{ translate('Generate Payment') }}</label>
                                <button id="createInvoiceButton" class="btn btn-primary"
                                    @if ($order->payment_status == 'paid' || $order->delivery_status == 'cancelled') disabled @endif>
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td id="shippingCost">{{ format_price($order->shipping_cost) }}</td>
                                    </tr>
                                    <tr>
                                        <td>
                                    ...
                                    <tr>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-primary"
                                                onclick="$('#updateShippingCostModal').modal('show')">
                                                {{ translate('Update Shipping Cost') }}
                                            </button>
                                        </td>
                                    </tr>
                                    @endif
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('orders.add_tracking_information') }}" method="POST">
                        ...
                        <div class="form-group mb-1">
                            <label class="mb-0">{{ translate('Checkout Link') }}:</label>
                            <input type="text" class="form-control form-control-sm" name="checkout_link" id="checkout_link"
                                value="{{ $order->payment ? $order->payment->checkout_link : '' }}" required>
                        </div>
                        <div class="form-group mb-1">
...
@section('modal')
    ...
    <div id="updateShippingCostModal" class="modal fade">
        <div class="modal-dialog modal-md modal-dialog-centered modal-dialog-zoom">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title h6">{{ translate('Update Shipping Cost') }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                </div>
                <form id="updateShippingCostForm" action="{{ route('orders.update_shipping_cost') }}" method="POST">
                    @csrf
                    <input type="hidden" name="order_id" value="{{ $order->id }}">
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="shipping_cost">{{ translate('Shipping Cost') }}</label>
...
@section('script')
    <script type="text/javascript">
        ...
        document.getElementById('updateShippingCostForm').addEventListener('submit', function(event) {
            event.preventDefault();
            var form = this;
            var shippingCost = parseFloat(document.getElementById('shipping_cost').value);

            // Perform AJAX request to update the shipping cost
            fetch(form.action, {
                method: 'POST',
                ...
                if (data.success) {
                    ...
                    // document.getElementById('orderDetailsTotal').textContent = data.orderDetailsTotal.toFixed(2);
                    // document.getElementById('totalTax').textContent = data.totalTax.toFixed(2);
                    document.getElementById('shippingCost').textContent = data.shippingCost;
                    ...
                    $('#updateShippingCostModal').modal('hide');
                } else {
        ...
        $('#createInvoiceButton').on('click', function() {
            ...
            var amount = $('#grandTotal').text().trim();
            var payerEmail = '{{ $order->user->email }}';
            ...
            $.ajax({
                url: '{{ route("xendit-create-invoice") }}',
                type: 'POST',
                dataType: 'json',
                ...
                success: function(response) {
                    ...
                    document.getElementById('checkout_link').value = response.checkout_link;
                    document.getElementById('createInvoiceButton').disabled = true;
                    document.getElementById('updateShippingCostForm').style.display = 'none';
                    // alert('Invoice created: ' + response.checkout_link);
                    // Optionally, you can redirect the user to the checkout link
