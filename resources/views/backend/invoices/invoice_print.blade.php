...
<body>
    <div>
                                ...
                                <tbody>
                                    <tr>
                                        <td>
                                            ...
                                            @else
                                                <img src="{{ static_asset('assets/img/logo.webp') }}" height="30"
        ...
        @if ($order->payment_status != 'paid')
            <div style="margin:0 8px;border:1px solid #DEDEDE;border-radius:3px;padding:0 7px; clear:both;margin-top:15px;">
                <table>
                    <tbody>
                        <tr>
                            <td class="bold">{{ translate('Payment Status') }}:</td>
                            <td class="">{{ translate('Unpaid') }}</td>
                        </tr>
                        <tr>
                            <td class="bold">{{ translate('Payment Link') }}:</td>
                            <td class="">
                                @if($order->payment)
                                    <a href="{{ $order->payment->checkout_link }}" target="_blank">{{ $order->payment->checkout_link }}</a>
                                @endif
                            </td>
                        </tr>
                    </tbody>
        ...
        @endif
        @if ($order->combined_order->invoice != null && $order->payment_status == 'paid')
        <div style="margin:0 8px;border:1px solid #DEDEDE;border-radius:3px;padding:0 7px; clear:both;margin-top:15px;display:flex;justify-content: space-between;">
            <table>
            <tbody>
            <tr>
                <td class="bold">{{ translate('Payment Method') }}:</td>
                <td class="">{{ $order->combined_order->invoice->payment_method }}</td>
            </tr>
            <tr>
                <td class="bold">{{ translate('Bank Code') }}:</td>
                <td class="">{{ $order->combined_order->invoice->bank_code }}</td>
            </tr>
            <tr>
                <td class="bold">{{ translate('Paid Amount') }}:</td>
                <td class="">{{ format_price($order->combined_order->invoice->paid_amount) }}</td>
            </tr>
            <tr>
                <td class="bold">{{ translate('Payer Email') }}:</td>
                <td class="">{{ $order->combined_order->invoice->payer_email }}</td>
            </tr>
            <tr>
                <td class="bold">{{ translate('Paid At') }}:</td>
                <td class="">{{ $order->combined_order->invoice->paid_at }}</td>
            </tr>
            </tbody>
            </table>
        </div>
        ...
        <div
            style="margin:0 8px;border:1px solid #DEDEDE;border-radius:3px;padding:0 7px; clear:both;margin-top:15px;">
            <table class="padding">
                <thead>
                    <tr>
                        <td width="5%" class="text-left bold">{{ translate('S/L') }}</td>
                        <td width="35%" class="text-left bold">{{ translate('Product Name') }}</td>
                        <td width="25%" class="text-left bold">{{ translate('Unit Price') }}</td>
                        <td width="5%" class="text-left bold">{{ translate('Qty') }}</td>
                        <!-- <td width="30%" class="text-left bold">{{ translate('Unit Tax') }}</td> -->
                        <td width="20%" class="text-right bold">{{ translate('Total') }}</td>
                    </tr>
                </thead>
            </table>
        </div>
        <div style="margin:0 8px; padding:0 7px; clear:both;margin-top:15px;">
            <table class="lg-padding" style="border-collapse: collapse ;">
                ...
                <tbody class="strong ss">
                    @foreach ($order->orderDetails as $key => $orderDetail)
                        @if ($orderDetail->product != null)
                            <tr>
                                <td width="5%" class="text-left" style="border-bottom:1px solid #DEDEDE;padding-left:20px">{{ $key + 1 }}</td>
                                <td width="35%" class="text-left" style="border-bottom:1px solid #DEDEDE;">
                                ...
                                <td width="25%" class="text-left" style="border-bottom:1px solid #DEDEDE;">
                                    {{ format_price($orderDetail->price) }}</td>
                                <td width="5%" class="text-left" style="border-bottom:1px solid #DEDEDE;">
                                        {{ $orderDetail->quantity }}</td>
                                <!-- <td class="" style="border-bottom:1px solid #DEDEDE;">
                                    {{ format_price($orderDetail->tax) }}</td> -->
                                <td width="20%" class="text-right bold" style="border-bottom:1px solid #DEDEDE;padding-right:20px;">
