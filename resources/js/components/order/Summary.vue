  ...
  <div v-if="!is_empty_obj(orderDetails) && orderDetails.orders.length > 0">
    ...
    >
      ...
      class="re-payment fs-12 c-pointer text-primary ml-4"
        ...
        <div v-if="orderDetails.orders[0].payment_status == 'unpaid'"
        class="re-payment fs-12 c-pointer text-primary ml-4"
        @click.stop="redirectToWhatsApp(orderDetails)"
        > {{$t('pay_now')  }}</div>
...
export default {
  ...
  computed: {
    ...mapGetters("app", ["appName","generalSettings","appUrl"]),
  },
  mounted   () {
    // console.log('contact_phone', this.generalSettings.contact_phone)
  },
  props: {
  ...
  methods:{
    ...
    redirectToWhatsApp(orderDetails) {
        const adminPhoneNumber = this.generalSettings.contact_phone.replace(/[+\-\s]/g, '');
        const userName = orderDetails.user.name;
        const order = orderDetails.orders[0];
        const orderItems = order.products.data.map(product => `- ${product.name} (x${product.quantity}): ${product.price}`).join('\n'); // Disabled format_price for now
        const orderTotal = orderDetails.grand_total; // Disabled format_price for now
        const orderId = orderDetails.code;
        const message = encodeURIComponent(`Order details:\n\nOrder ID: ${orderId}\nName: ${userName}\n\nOrder Summary:\n${orderItems}\n\nTotal: ${orderTotal}`);
        window.location.href = `https://wa.me/${adminPhoneNumber}?text=${message}`;
